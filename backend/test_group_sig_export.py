...
out_path = os.path.join(base_dir, "backend", "downloads", "test_group_sig_export.doc")
...
# 2 contractors, 1 has 2 parcels, 1 has 1 parcel
group_rows = [
    {
...
# Find segments for contractor signature merge
segments = []
curr_cbfbm = None
start_r = 2
for i, r in enumerate(group_rows):
...
# Create a test sig image for 张三
sig_dir = os.path.join(base_dir, "backend", "uploads", "signatures")
os.makedirs(sig_dir, exist_ok=True)
sig_zhangsan = os.path.join(sig_dir, "3411241002000010001.png")
shutil.copy(os.path.join(sig_dir, "test_sig_0123.png"), sig_zhangsan)
...
t8.Rows(r_stat_idx).Cells(2).Range.Text = str(total_errors)
t8.Rows(r_stat_idx).Cells(3).Range.Text = str(satisfaction_count)

r_score_idx = total_count + 3
t8.Rows(r_score_idx).Cells(1).Range.Text = f"发包方程序规范得分=20-发包方错误总和({total_errors})×0.5={prog_score:.1f}；发包方工作成效（满意度调查）得分=满意数({satisfaction_count})/抽检数({total_count})×10={effect_score:.1f}"

doc8.SaveAs2(FileName=out_path, FileFormat=0)
doc8.Close(0)
word.Quit()
pythoncom.CoUninitialize()
print("Export with merged contractor signature passed successfully!")