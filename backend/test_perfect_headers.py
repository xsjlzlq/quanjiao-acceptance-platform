...
out_path = os.path.join(base_dir, "backend", "downloads", "test_perfect_headers.doc")
...
for idx in range(1, doc.Tables.Count + 1):
    ...
    header_p.Range.Text = f"行政区划名称：{full_area_name:<16}  验收内容：{c_name:<16}  2026 年       月      日\r"

# Verify all 4 tables preceding paragraph
with open(r"G:\全椒县二轮延包\全椒县县级验收管理平台\backend\perfect_headers_res.txt", "w", encoding="utf-8") as out:
    for idx in range(1, doc.Tables.Count + 1):
        t = doc.Tables(idx)
        pre_range = doc.Range(0, t.Range.Start)
        header_p = pre_range.Paragraphs(pre_range.Paragraphs.Count)
...
print("Perfect headers test finished!")