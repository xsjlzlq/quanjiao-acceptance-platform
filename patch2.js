...
if (!text.includes('const initExportPickerColumns = async () => {') && !text.includes('const initExportPickerColumns = () => {')) {
    text = text.replace(/onUnmounted\(\(\) => \{[\s\S]*?\}\);\s*/, match => match + newFunc);
}

text = text.replace(/await await/g, 'await');
text = text.replace(/initExportPickerColumns\(\)/g, 'await initExportPickerColumns()');
text = text.replace(/await await/g, 'await');
