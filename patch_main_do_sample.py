...
# Find the start of do_sample and generate_att4
import re
match = re.search(r'@app.post\("/api/sample"\).*?def generate_att4\(\):', code, re.DOTALL)

if match:
    # Let's just redefine the whole block manually below
    pass
...
# Replace in code
head = code.split('@app.post("/api/sample")')[0]
tail = code.split('def generate_att4(')[1].split('\n', 1)[1] # skip def line

# Also we need to keep the bottom sync-waiye endpoint, so just split properly.
bottom = "\n" + code.split('@app.post("/api/sync-waiye")')[0].split('class WaiyeRecord')[1]

# Better way: replace the exact range
import re
code = re.sub(r'@app.post\("/api/sample"\).*?@app.post\("/api/sync-waiye"\)', new_block + '\n\nclass WaiyeRecord(BaseModel):\n' + bottom.split('class WaiyeRecord(BaseModel):\n')[1], code, flags=re.DOTALL)
